package by.it.group251003.stasevich_uriu.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for(int i = 0; i < st.length(); i++)
             count.put(st.charAt(i), count.get(st.charAt(i)) !=null ? count.get(st.charAt(i))+1:1);
        ...
        for(char c: count.keySet())
            pQueue.add(new LeafNode(count.get(c), c));
        //3. вынимая по два узла из очереди (для сборки родителя)
        //и возвращая этого родителя обратно в очередь
        //построим дерево кодирования Хаффмана.
        //У родителя частоты детей складываются.
        while(pQueue.size()>1){
            InternalNode parent = new InternalNode(pQueue.remove(), pQueue.remove());
            pQueue.add(parent);
        }
        //4. последний из родителей будет корнем этого дерева
        //это будет последний и единственный элемент оставшийся в очереди priorityQueue.
        pQueue.element().fillCodes("");
        StringBuilder sB = new StringBuilder();
        for(int i = 0; i< st.length(); i++)
            sB.append(codes.get(st.charAt(i)));
        return sB.toString();
        //.....
