package by.it.group251003.stasevich_uriu.lesson12;
...
public class MyRbMap implements SortedMap<Integer, String> {
    ...
    private Node moveRedL(Node node) {
        flipColors(node);
    ...
    private Node moveRedR(Node node) {
        flipColors(node);
    ...
    public void printTree(Node node, StringBuilder stringBuilder){
        ...
        stringBuilder.append(node.key).append("=").append(node.info).append(", ");
    ...
    private void setToKey(Node node, int toKey, SortedMap<Integer, String> sortedMap) {
        ...
        if (node.key < toKey) {
            sortedMap.put(node.key, node.info);
