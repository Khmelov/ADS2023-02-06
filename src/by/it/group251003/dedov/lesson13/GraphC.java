package by.it.group251003.dedov.lesson13;
...
public class GraphC {
    ...
    private static void dfs (Map < String, ArrayList < String >> neighbours, String
        v, Set < String > vis, Stack < String > st){
            vis.add(v);
    ...
    private static void dfs (Map < String, ArrayList < String >> neighbours, String
            v, Set < String > vis, StringBuilder sb){
        vis.add(v);
        sb.append(v);
        if (neighbours.get(v) != null) {
            for (String child : neighbours.get(v)) {
