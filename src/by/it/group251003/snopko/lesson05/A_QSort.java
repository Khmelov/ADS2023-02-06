...
import by.it.group251003.snopko.lesson02.B_Sheduler;
...
public class A_QSort {
    ...
    private class Segment  implements Comparable<Segment>{
        ...
        public int compareTo(Segment o) {
            ...
            if((this.stop> o.stop) || (this.stop == o.stop) && (this.start > o.start)){return 1;}
            return -1;
    ...
    Segment[] QuickSort(Segment[] arr, int left, int right){
        if (right > left) {
            Segment ideal = arr[(left + right) / 2];
            int i = left, j = right;
            while (i < j) {
                while (arr[i].stop < ideal.stop){
                    i++;
                }
                while (arr[j].stop > ideal.stop) {
                    j--;
                }
                if (i < j) {
                    Segment bank = arr[i];
