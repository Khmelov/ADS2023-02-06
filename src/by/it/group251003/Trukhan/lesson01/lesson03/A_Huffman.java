package by.it.group251003.Trukhan.lesson01.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0;  i < s.length(); i++) {
            char El = s.charAt(i);
            if(count.containsKey(El)) count.put(El, count.get(El) + 1);
            else count.put(El, 1);
        }

        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> el : count.entrySet()) {
            Node node = new LeafNode(el.getValue(), el.getKey());
            priorityQueue.add(node);
        }

        Node el = null;
        while(priorityQueue.size() != 1) {
            el = new InternalNode(priorityQueue.poll(), priorityQueue.poll());
            priorityQueue.add(el);
        }
        el.fillCodes("");
        StringBuilder sb = new StringBuilder();

        for (int i = 0; i < s.length(); i++) sb.append(codes.get(s.charAt(i)));
        //.....
