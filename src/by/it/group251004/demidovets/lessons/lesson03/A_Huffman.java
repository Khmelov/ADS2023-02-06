package by.it.group251004.demidovets.lessons.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++)
            count.put(s.charAt(i), count.get(s.charAt(i)) != null ? count.get(s.charAt(i)) + 1 : 1);
        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>(); //И есть Array List?
        for (char c: count.keySet())
            priorityQueue.add(new LeafNode(count.get(c), c)); //В очередь занесли листы нашего будущего дерева, первое – частота элемента, 2 — сама буква
