...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < values.size(); i++) {
            A_Huffman.LeafNode temp = new A_Huffman.LeafNode(values.get(i), keys.get(i));
            priorityQueue.add(temp);
        }
        //2. перенесем все символы в приоритетную очередь в виде листьев

        while(priorityQueue.size() != 1){
            A_Huffman.Node son1 = priorityQueue.poll();
            A_Huffman.Node son2 = priorityQueue.poll();
            if (son1 != null && son2 != null) {
                A_Huffman.InternalNode temp = new A_Huffman.InternalNode(son1, son2);
                priorityQueue.add(temp);
            }
        }
